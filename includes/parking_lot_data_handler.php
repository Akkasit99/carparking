<?php
// parking_lot_data_handler.php - จัดการข้อมูลรถที่เข้าลานจอด

function getParkingLotData($selected_date, $selected_time, $supabase_url, $supabase_key) {
...
function renderParkingLotTable($result, $selected_date, $selected_time) {
    $imagesData = [];
    ...
    if (is_array($result) && count($result) > 0) {
        foreach($result as $row) {
            ...
            echo '<img src="' . htmlspecialchars($imgUrl) . '" width="120" style="max-height: 90px; object-fit: cover;" onclick="openModal(\'' . htmlspecialchars($imgUrl) . '\')">'; 
            echo '</td>';
