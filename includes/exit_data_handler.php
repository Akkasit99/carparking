<?php
// exit_data_handler.php - จัดการข้อมูลรถทางออก

function getExitData($selected_date, $selected_time, $supabase_url, $supabase_key) {
    ...
    if ($selected_date == date('Y-m-d') && $selected_time == '00:00') {
        ...
        $url = $supabase_url . "/rest/v1/parking_exit?select=id,image,date,camera_id"
...
function renderExitTable($result, $selected_date, $selected_time) {
    $imagesData = [];
    ...
    if (is_array($result) && count($result) > 0) {
        ...
        if ($selected_date == date('Y-m-d') && $selected_time == '00:00') {
            echo '<td colspan="4"><div class="summary-content"><i class="fas fa-info-circle"></i> แสดงรูปภาพทั้งหมดในฐานข้อมูล <strong>' . count($result) . '</strong> รายการ</div></td>';
        } else {
            echo '<td colspan="4"><div class="summary-content"><i class="fas fa-info-circle"></i> พบข้อมูลในวันที่เลือก <strong>' . count($result) . '</strong> รายการ</div></td>';
        }
        echo '</tr>';
    } else {
        echo '<tr class="no-data-row"><td colspan="4"><div class="no-data-content"><i class="fas fa-exclamation-circle"></i> ไม่พบข้อมูล</div></td></tr>';
    }
